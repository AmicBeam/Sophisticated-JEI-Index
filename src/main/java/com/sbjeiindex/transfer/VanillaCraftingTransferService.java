...
public final class VanillaCraftingTransferService {
    ...
    public static void transfer(ServerPlayer player, VanillaRecipeTransferPayload payload) {
        ...
        Optional<? extends Recipe<?>> recipe = player.serverLevel().getRecipeManager().byKey(payload.recipeId());
        if (recipe.isEmpty() || !(recipe.get() instanceof CraftingRecipe craftingRecipe)) {
            return;
        }
        List<ItemStack> templates = payload.templates().stream()
            .map(stack -> stack.isEmpty() ? ItemStack.EMPTY : copyWithCount(stack, 1))
            .toList();
        if (!matches(craftingRecipe, templates, menu, player)) {
            return;
        }
        ...
        if (!matches(craftingRecipe, currentGrid(menu), menu, player)) {
            transaction.rollback();
            return;
        }
    ...
    private static boolean matches(CraftingRecipe recipe, List<ItemStack> stacks, CraftingMenu menu, ServerPlayer player) {
        TransientCraftingContainer input = new TransientCraftingContainer(menu, 3, 3);
        for (int slot = 0; slot < 9; slot++) {
            input.setItem(slot, stacks.get(slot).copy());
        }
    ...
    private static final class Transaction {
        ...
        private boolean apply(List<ItemStack> templates, int sets) {
            ...
            for (int index = 0; index < 9; index++) {
                ...
                ItemStack target = copyWithCount(template, sets);
        ...
        private ItemStack insertIntoMenu(ItemStack stack) {
            ...
            for (int slot = 10; slot <= 45 && !remaining.isEmpty(); slot++) {
                ...
                if (target.getItem().isEmpty() && target.mayPlace(remaining)) {
                    ...
                    target.set(copyWithCount(remaining, moved));
                    remaining.shrink(moved);
                }
            }
            return remaining;
