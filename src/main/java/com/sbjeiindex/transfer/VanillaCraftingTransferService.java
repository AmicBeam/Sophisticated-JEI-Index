...
public final class VanillaCraftingTransferService {
    ...
    public static void transfer(ServerPlayer player, VanillaRecipeTransferPayload payload) {
        ...
        Optional<RecipeHolder<?>> recipe = player.level().recipeAccess().byKey(payload.recipeId());
        ...
        List<ItemStack> templates = payload.templates().stream()
            ...
            .toList();
        if (!craftingRecipe.matches(CraftingInput.of(3, 3, templates), player.level())) {
            return;
        }
        ...
        if (!craftingRecipe.matches(CraftingInput.of(3, 3, currentGrid(menu)), player.level())) {
            transaction.rollback();
            return;
        }
    ...
    private static final class Transaction {
        ...
        private Transaction(CraftingMenu menu, List<InventoryHandler> backpacks) {
            ...
            for (InventoryHandler handler : backpacks) {
                List<ItemStack> snapshot = new ArrayList<>(handler.size());
            ...
            for (InventoryHandler handler : backpacks) {
                for (int slot = 0; slot < handler.size() && remaining > 0; slot++) {
                    if (same(handler.getStackInSlot(slot), template)) {
                        ...
                        left.shrink(taken);
                        handler.setStackInSlot(slot, left);
                        remaining -= taken;
                    }
                }
            }
        ...
        private ItemStack insertIntoBackpacks(ItemStack stack) {
            ...
            for (InventoryHandler handler : backpacks) {
                for (int slot = 0; slot < handler.size() && !remaining.isEmpty(); slot++) {
                    ...
                    if (!present.isEmpty() && same(present, remaining) && handler.isItemValid(slot, remaining)) {
                        int moved = Math.min(remaining.getCount(), handler.getInternalSlotLimit(slot) - present.getCount());
                        if (moved > 0) {
                            ItemStack grown = present.copy();
                            grown.grow(moved);
                ...
                for (int slot = 0; slot < handler.size() && !remaining.isEmpty(); slot++) {
                    if (handler.getStackInSlot(slot).isEmpty() && handler.isItemValid(slot, remaining)) {
                        int moved = Math.min(remaining.getCount(), handler.getInternalSlotLimit(slot));
