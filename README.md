...
- ✅ Also supports the crafting terminals of the following mods (priority: network → player inventory → enabled backpacks in order): AE2, Refined Storage, Tom's Storage (JEI only), Beyond Dimensions (JEI only)
...
- Tom's Storage and Beyond Dimensions are supported via JEI recipe transfer only. EMI is not supported.
